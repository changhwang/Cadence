...
function renderDietView(container) {
    ...
    // 3. Compact Nutrition Dashboard (V19: 4 Donuts + Water)
    ...
    // Calculate Ring Data
    const createRing = (label, current, max, color, size = 70) => {
        const pct = Math.min(100, (current / max) * 100);
        const deg = (pct / 100) * 360;
        return `
            <div style="display:flex; flex-direction:column; align-items:center;">
                <div style="position:relative; width:${size}px; height:${size}px; border-radius:50%; background:conic-gradient(${color} ${deg}deg, #f2f2f7 0deg); display:flex; align-items:center; justify-content:center;">
                    <div style="width:${size - 12}px; height:${size - 12}px; background:white; border-radius:50%; display:flex; align-items:center; justify-content:center;">
                        <span style="font-size:${size / 4}px; font-weight:700;">${Math.round(current)}</span>
                    </div>
                </div>
                <div style="font-size:12px; color:gray; margin-top:5px; font-weight:600;">${label}</div>
            </div>
        `;
    };

    // Row 1: Calories (Big Ring) + Water (Card style but clean)
    ...
    dashboard.innerHTML = `
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
            ...
            <div style="display:flex; align-items:center; gap:20px;">
                 <div style="position:relative; width:90px; height:90px; border-radius:50%; background:conic-gradient(var(--ios-blue) ${calDeg}deg, #f2f2f7 0deg); display:flex; align-items:center; justify-content:center;">
                    <div style="width:74px; height:74px; background:white; border-radius:50%; display:grid; place-items:center;">
                        <i data-lucide="flame" style="width:24px; color:var(--ios-blue);"></i>
                    </div>
                 </div>
                 <div>
                    <div style="font-size:14px; color:gray; font-weight:600; margin-bottom:2px;">Calories</div>
                    <div style="font-size:20px; font-weight:800;">
                        ${Math.round(total.cal)} <span style="font-size:14px; color:gray; font-weight:500;">/ ${settings.targetCal}</span>
                    </div>
                 </div>
            </div>

            <!-- Water Card (Simple V19) -->
            <div style="background:#f2f4f7; padding:15px; border-radius:16px; min-width:120px; text-align:center;">
                <div style="display:flex; justify-content:center; align-items:center; gap:5px; margin-bottom:5px;">
                     <i data-lucide="droplet" style="width:18px; color:#5AC8FA;"></i>
                     <span style="font-weight:700; color:#555;">Water</span>
                </div>
                <div style="font-size:16px; font-weight:700; color:#5AC8FA;">
                    ${water} <span style="font-size:12px; color:gray;">/ ${settings.targetWater}ml</span>
                </div>
            </div>
        </div>

        <!-- Row 2: Macros Donuts -->
        <div style="display:flex; justify-content:space-around; align-items:flex-end; padding-top:10px; border-top:1px solid #f5f5f5;">
            ${createRing("Protein", total.pro, settings.targetPro, "var(--ios-blue)", 60)}
            ${createRing("Carbs", total.carbo, 300, "var(--ios-orange)", 60)}
            ${createRing("Fat", total.fat, 80, "#FFCC00", 60)}
        </div>
    `;
...
// 1.1 Water Modal (V19 Fix)
window.openWaterModal = () => {
    const modal = createModal('Log Water');
    modal.className = 'modal-overlay center open';
    modal.innerHTML = `
        <div style="background:white; padding:25px; border-radius:24px; width:80%; max-width:320px; text-align:center;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:20px;">
                <button onclick="logWater(250); closeModal();" style="padding:15px; background:#f2f4f7; border:none; border-radius:12px; font-weight:600; color:#5AC8FA;">+250ml</button>
                <button onclick="logWater(500); closeModal();" style="padding:15px; background:#f2f4f7; border:none; border-radius:12px; font-weight:600; color:#5AC8FA;">+500ml</button>
            </div>

            <button onclick="closeModal()" style="background:none; border:none; color:gray;">Close</button>
        </div>
    `;
    lucide.createIcons();
}
