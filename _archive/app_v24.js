...
window.openAddFoodMenu = (mealType) => {
    ...
    const categories = ['전체', '한식', '일식/중식', '단백질', '탄수화물', '소스/양념', '가공식품'];
    const filterRules = {
        '전체': (k) => true,
    ...
    const render = () => {
        ...
        const tabsHtml = categories.map(cat => `
            <button onclick="window.updateFoodCategory('${cat}')" 
        ...
        const listHtml = matched.map(key => {
            const item = FOOD_DB[key];
            return `
                <div onclick="openMealDetail('${mealType}', '${key}')" style="padding:12px; border-bottom:1px solid #f5f5f5; cursor:pointer;">
                    <div style="font-size:15px; font-weight:500; color:#333; margin-bottom:2px;">${key}</div>
                    <div style="font-size:12px; color:gray;">${item.cal} kcal / ${item.default_gram}${item.unit}</div>
                </div>
            `;
        }).join('');
        ...
        modal.innerHTML = `
            <div style="padding:10px;">
                <!-- Search -->
                <input type="text" value="${state.term}" oninput="window.updateFoodSearch(this.value)" placeholder="Search food..." style="width:100%; padding:12px; border:1px solid #ddd; border-radius:10px; margin-bottom:10px; font-family:inherit;">
                
                <!-- Category Tabs -->
                <div style="overflow-x:auto; white-space:nowrap; -webkit-overflow-scrolling:touch; margin-bottom:10px; padding-bottom:5px;">
                    ${tabsHtml}
                </div>

                <!-- Results -->
                <div style="height:50vh; overflow-y:auto; border-top:1px solid #eee;">
                    ${matched.length === 0 ? '<div style="padding:20px; text-align:center; color:gray;">No results found</div>' : listHtml}
                    ${resultCount > 100 ? `<div style="padding:10px; text-align:center; color:gray; font-size:12px;">Showing top 100 of ${resultCount} results</div>` : ''}
                </div>
            </div>
        `;
    ...
    // Attach state to window for event handlers (temporary closure workaround)
    window._tempFoodState = state;
    window._tempFoodRender = render;

    window.updateFoodCategory = (cat) => {
        window._tempFoodState.category = cat;
        window._tempFoodRender();
    };

    window.updateFoodSearch = (val) => {
        window._tempFoodState.term = val.toLowerCase();
        window._tempFoodRender();
    };
...
window.openAddFoodMenu = (mealType) => {
    // 1. Create Modal with Standard Header
    const modal = createModal('Add Food');

    // 2. Create Body Container to avoid overwriting Header
    const body = document.createElement('div');
    ...
    const render = () => {
        ...
        const tabsHtml = categories.map(cat => `
            <button onclick="window.updateFoodCategory('${cat}')" 
                style="padding:8px 14px; margin-right:6px; margin-bottom:8px; border-radius:18px; border:none; font-size:13px; font-weight:600; cursor:pointer; 
                background:${state.category === cat ? '#333' : '#f0f0f5'}; color:${state.category === cat ? 'white' : '#555'}; transition:all 0.2s;">
                ${cat}
            </button>
        `).join('');
        ...
        const listHtml = matched.map(key => {
            const item = FOOD_DB[key];
            return `
                <div onclick="openMealDetail('${mealType}', '${key}')" style="padding:12px 0; border-bottom:1px solid #f5f5f5; cursor:pointer; display:flex; justify-content:space-between; align-items:center;">
                    <div style="font-size:15px; font-weight:500; color:#333;">${key}</div>
                    <div style="font-size:13px; color:gray;">${item.cal} kcal</div>
                </div>
            `;
        }).join('');
        ...
        body.innerHTML = `
            <div style="padding:0 5px 10px 5px;">
                <!-- Search -->
                <input type="text" value="${state.term}" oninput="window.updateFoodSearch(this.value)" placeholder="Search food..." style="width:100%; padding:12px; border:1px solid #ddd; border-radius:10px; margin-bottom:15px; font-family:inherit; font-size:16px;">
                
                <!-- Category Tabs -->
                <div style="overflow-x:auto; white-space:nowrap; -webkit-overflow-scrolling:touch; margin-bottom:15px; padding-bottom:5px;">
                    ${tabsHtml}
                </div>

                <!-- Results -->
                <div style="height:50vh; overflow-y:auto; border-top:1px solid #eee;">
                    ${matched.length === 0 ? '<div style="padding:40px; text-align:center; color:gray;">검색 결과가 없습니다</div>' : listHtml}
                    ${resultCount > 100 ? `<div style="padding:15px; text-align:center; color:gray; font-size:12px;">Top 100 of ${resultCount}</div>` : ''}
                </div>
            </div>
        `;
...
window.openFoodSelector = () => {
    ...
    const categories = ['All', '한식', '일식/중식', '단백질', '탄수화물', '소스/양념', '가공식품'];
    const filterRules = {
        'All': (k) => true,
        '한식': (k) => /밥|국|찌개|김치|비빔|전|탕|반찬|나물/.test(k),
        '일식/중식': (k) => /초밥|우동|라멘|돈까스|스시|짬뽕|자장|탕수육|마라|딤섬|소바/.test(k),
...
window.openFoodSelector = () => {
    ...
    const filterApi = (key, item) => {
        ...
        // Exact match or fallback logic
        if (item.category === cat) return true;

        // Compatibility for items without category (Legacy fallback)
        if (!item.category) {
            // Include in '전체' only
            return false;
        }

        // Grouping logic (if tabs combine DB categories)
...
window.openExerciseModal = (initialFilter = null) => {
    ...
    const categories = ['Chest', 'Back', 'Shoulders', 'Arms', 'Legs', 'Core', 'Cardio']; // Added Cardio/Core
...
// --- V25 Feature: Time-Series Goal History (Append) ---
...
window.GoalManager = {
    ...
    // Migrate legacy user_settings to history on first run
    migrate: () => {
        const history = Store.get('goal_history', null);
        if (!history || history.length === 0) {
            const current = Store.get('user_settings', { targetCal: 2500, targetPro: 160, targetWater: 2000, targetSodium: 2000 });
            // Create initial entry for the "beginning of time"
            const initialEntry = {
                date: '2000-01-01',
                targets: { ...current }
            };
            Store.set('goal_history', [initialEntry]);
            console.log("[GoalManager] Migrated legacy settings to history.");
        }
    },

    // Get effective goals for a specific date (YYYY-MM-DD or YYYY. M. D.)
    // Returns object: { targetCal, targetPro, etc }
    getGoals: (dateStr) => {
        // Normalize date to YYYY-MM-DD for comparison
        // state.date format is often "2024. 1. 1." or "2024-01-01"
        // Let's standarize to string comparison.
        // Assuming dateStr matches format in history or is comparable.
        // Actually best to compare as standardized strings.

        const history = Store.get('goal_history', []);
        if (history.length === 0) return Store.get('user_settings', {});

        // Normalize input date
        // Simple normalization: just string compare if format matches. 
        // If formats differ, we need more robust parsing.
        // Current App uses YYYY-MM-DD for Internal keys mostly? No, state.date is locale string "2024. 1. 1.".
        // WAIT. app_v24.js uses "new Date().toLocaleDateString('ko-KR')" => "2024. 1. 1."
        // We should save history using this format for consistency OR standard ISO.
        // Let's use Standard ISO YYYY-MM-DD for storage, and convert input.

        // Helper to convert "2024. 1. 1." -> "2024-01-01"
        const normalize = (d) => {
            ...
            // If "2024. 1. 1."
            const parts = d.replace(/\.$/, '').split('.').map(s => s.trim());
            if (parts.length === 3) {
                const y = parts[0];
        ...
        const targetDate = normalize(dateStr);

        // Find latest entry <= targetDate
        // History should be sorted date ASC
        let effective = history[0]; // Default to first (oldest)

        for (let i = 0; i < history.length; i++) {
            const hDate = normalize(history[i].date);
            if (hDate <= targetDate) {
                effective = history[i];
            } else {
                break; // Because sorted ASC, once we pass targetDate, previous was the one.
            }
        }
    ...
    // Save new goals effective from TODAY (or specified date)
    save: (newTargets) => {
        const history = Store.get('goal_history', []);

        // Use YYYY-MM-DD for robust storage
        const today = new Date();
        const y = today.getFullYear();
        const m = String(today.getMonth() + 1).padStart(2, '0');
        const d = String(today.getDate()).padStart(2, '0');
        const dateKey = `${y}-${m}-${d}`; // Standard ISO

        // Remove existing entry for today if exists
        const cleanHistory = history.filter(h => {
            // Normalize stored date just in case
            return h.date.replace(/\./g, '-').replace(/ /g, '') !== dateKey; // Simple check? 
            // Better:
            // const hDate = ... (normalization irrelevant if we stick to ISO)
            // Let's assume we store ISO.
        ...
        // Sort
        cleanHistory.sort((a, b) => a.date.localeCompare(b.date));

        Store.set('goal_history', cleanHistory);

        // Also update legacy cache for safety
        Store.set('user_settings', newTargets);
...
try {
    window.GoalManager.migrate();
} catch (e) { console.error(e); }

// 2. Overide renderSettings logic to use GoalManager
// (We need to replace the save function inside renderSettings or overwrite renderSettings)
// Since renderSettings is large, we might replacing the saveHandler via global hook if possible? 
// Or just overwrite renderSettings. Overwriting is safer for V25.

window.renderSettings = (container) => {
    // Load current goals
    const goals = GoalManager.getGoals(state.date); // Show goals relevant to VIEWING date? 
    // Usually Settings shows "Current Configuration". 
    // Let's show Today's goals (or latest).
    const latestGoals = Store.get('user_settings') || goals;

    // ... (UI Code similar to V24 but simpler call to save) ...
    // Let's retain the V24 Settings UI but change the Save Action.

    container.innerHTML = `
        <div style="padding-top:20px; padding-bottom:100px;">
             <!-- Header -->
             <div style="padding:20px; font-size:24px; font-weight:800;">Settings</div>
             
             <!-- Profile Section (Read Only / Edit via Modal usually) -->
             <div class="card" style="margin:20px;">
                <h3 style="margin-bottom:15px;">Target Goals (Daily)</h3>
                
                <div style="display:grid; gap:15px;">
                    <div>
                        <label style="color:gray; font-size:12px;">Calories (kcal)</label>
                        <input type="number" id="set_cal" value="${latestGoals.targetCal}" style="width:100%; padding:10px; border:1px solid #eee; border-radius:10px; font-size:16px;">
                    </div>
                    <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px;">
                        <div>
                             <label style="color:gray; font-size:12px;">Carbs (g)</label>
                             <input type="number" id="set_carb" value="${latestGoals.targetCarb}" style="width:100%; padding:10px; border:1px solid #eee; border-radius:10px;">
                        </div>
                        <div>
                             <label style="color:gray; font-size:12px;">Protein (g)</label>
                             <input type="number" id="set_pro" value="${latestGoals.targetPro}" style="width:100%; padding:10px; border:1px solid #eee; border-radius:10px;">
                        </div>
                        <div>
                             <label style="color:gray; font-size:12px;">Fat (g)</label>
                             <input type="number" id="set_fat" value="${latestGoals.targetFat}" style="width:100%; padding:10px; border:1px solid #eee; border-radius:10px;">
                        </div>
                    </div>
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                         <div>
                             <label style="color:gray; font-size:12px;">Sodium (mg)</label>
                             <input type="number" id="set_sodium" value="${latestGoals.targetSodium || 2000}" style="width:100%; padding:10px; border:1px solid #eee; border-radius:10px;">
                        </div>
                         <div>
                             <label style="color:gray; font-size:12px;">Water (ml)</label>
                             <input type="number" id="set_water" value="${latestGoals.targetWater || 2000}" style="width:100%; padding:10px; border:1px solid #eee; border-radius:10px;">
                        </div>
                    </div>
                </div>
                
                <button onclick="saveTimeSettings()" style="width:100%; margin-top:20px; padding:15px; background:var(--ios-blue); color:white; border:none; border-radius:12px; font-weight:700;">Save New Goals</button>
                <div style="text-align:center; margin-top:10px; font-size:12px; color:gray;">Changes apply from today onwards</div>
             </div>
             
             <!-- Reset Data -->
             <div class="card" style="margin:20px; margin-top:0;">
                <h3 style="margin-bottom:15px; color:var(--ios-red);">Danger Zone</h3>
                <button onclick="if(confirm('Reset ALL data?')) { localStorage.clear(); location.reload(); }" style="width:100%; padding:15px; background:#fff0f0; color:var(--ios-red); border:none; border-radius:12px; font-weight:600;">Reset Everything</button>
             </div>
        </div>
    `;
};

window.saveTimeSettings = () => {
    const newTargets = {
        targetCal: parseInt(document.getElementById('set_cal').value) || 2000,
        targetPro: parseInt(document.getElementById('set_pro').value) || 100,
        targetCarb: parseInt(document.getElementById('set_carb').value) || 200,
        targetFat: parseInt(document.getElementById('set_fat').value) || 50,
        targetSodium: parseInt(document.getElementById('set_sodium').value) || 2000,
        targetWater: parseInt(document.getElementById('set_water').value) || 2000
    };

    GoalManager.save(newTargets);
    alert('Goals updated for today onwards!');
    render();
};

// --- V25 Fix: Bind Goal Logic to Global Functions ---

// Map legacy function to new Manager
// This overrides the implementation at Line 460
window.getGoalsForDate = (dateStr) => {
    return GoalManager.getGoals(dateStr);
};

// Also ensure renderSettings calls the new save function
// (The previous patch already redefined renderSettings and saveTimeSettings, 
// so this file just ensures getGoalsForDate logic is consistent for renderDietView)

console.log("[GoalManager] Time-Series Goals Logic Activated.");

// --- V24 Fix 6: Restore Diet Log Logic (Append) ---

// 1. Re-implement selectFoodItem (called by openFoodSelector)
window.selectFoodItem = (key) => {
    window.openFoodDetail(key);
};

// 2. Re-implement openFoodDetail (Sub-modal for amount)
window.openFoodDetail = (key) => {
    const item = FOOD_DB[key];
    const gram = item.default_g || item.default_gram || 100;
    ...
    modal.className = 'modal-overlay center open';
    modal.style.zIndex = '1100'; // Higher than selector

    modal.innerHTML = `
        <div style="background:white; padding:25px; border-radius:24px; width:85%; max-width:320px; text-align:center; box-shadow:0 10px 40px rgba(0,0,0,0.2);">
            <h3 style="margin:0 0 5px 0; font-size:18px;">${key}</h3>
            <div style="font-size:13px; color:gray; margin-bottom:20px;">${item.cal} kcal / ${gram}${unit}</div>
            
            <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px;">
                <button onclick="adjustFoodAmount(-10)" style="width:40px; height:40px; border-radius:12px; border:1px solid #eee; background:white;">-</button>
                <div style="flex:1;">
                   <input id="foodDetailAmount" type="number" value="${gram}" style="width:100%; text-align:center; font-size:24px; font-weight:700; border:none; color:var(--ios-blue);">
                   <div style="font-size:12px; color:gray;">${unit === 'g' ? 'gram' : unit}</div>
                </div>
                <button onclick="adjustFoodAmount(10)" style="width:40px; height:40px; border-radius:12px; border:1px solid #eee; background:white;">+</button>
            </div>

            <div style="font-size:14px; margin-bottom:20px; padding:10px; background:#f5f5f5; border-radius:12px;">
                Expected: <span id="foodDetailCal" style="font-weight:700;">${item.cal}</span> kcal
            </div>

            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                <button onclick="document.body.removeChild(document.getElementById('food-detail-modal'))" style="padding:15px; background:#f0f0f5; color:gray; border:none; border-radius:12px; font-weight:600;">Cancel</button>
                <button onclick="confirmAddFood('${key}')" style="padding:15px; background:var(--ios-blue); color:white; border:none; border-radius:12px; font-weight:600;">Add</button>
            </div>
        </div>
    `;
    ...
    // Helper to calculate live calories
    const input = document.getElementById('foodDetailAmount');
    const updateCal = () => {
        const val = parseFloat(input.value) || 0;
        const ratio = val / gram;
        document.getElementById('foodDetailCal').innerText = Math.round(item.cal * ratio);
    };

    input.oninput = updateCal;

    window.adjustFoodAmount = (delta) => {
...
// 3. Confirm Add (Push to tempMeal and Close Modals)
window.confirmAddFood = (key) => {
    const item = FOOD_DB[key];
    const amount = parseFloat(document.getElementById('foodDetailAmount').value) || 0;
    const gram = item.default_g || item.default_gram || 100;
    const ratio = amount / gram;
    ...
    // Add to Global Temp Meal Ref (Meal Builder)
    if (window.tempMealRef && window.tempMealRef.foods) {
        window.tempMealRef.foods.push(entry);
        console.log("Added food:", entry);

        // Refresh Editor
        if (window.renderEditorRef) window.renderEditorRef();

        // Close Detail Modal
        document.body.removeChild(document.getElementById('food-detail-modal'));

        // Close Selector Modal (Optional: keep open to add more? User usually wants to add more)
        // Let's keep Selector open! User flow: Add Rice -> Add Soup -> Add Kimchi -> Done.
        // So DO NOT close Selector.
    } else {
        alert("Error: Meal Builder session not found.");
        closeModal(); // Emergency exit
    }
};

// --- V25 Fix: Goal History Logic Update (Append) ---

// Override saveAllSettings to ensure consistent Date Format for History
window.saveAllSettings = () => {
    const { profile, settings, lang } = window.tempSettingsState;
    ...
    // --- Goal History Logic (Refined) ---
    const history = Store.get('goal_history', []);

    // Explicit Date Format: YYYY. M. D. (Consistent with state.date)
    const d = new Date();
    const realToday = `${d.getFullYear()}. ${d.getMonth() + 1}. ${d.getDate()}.`;

    // Remove existing entry for today to update it
    const newHistory = history.filter(h => h.date !== realToday);
    ...
    newHistory.sort((a, b) => parseDate(b.date) - parseDate(a.date));
...
// --- Debug Patch: Safe Render Diet View ---

// Override renderDietView with Try-Catch for Debugging
renderDietView = function (container) {
    try {
        console.log("Starting renderDietView");
        const dietKey = `diet_${state.date}`;
        const saved = Store.get(dietKey, { meals: [], water: 0 });
        const meals = saved.meals || [];
        const water = saved.water || 0;
        ...
        // 1. Stats Calculation
        const settings = window.getGoalsForDate(state.date);
        if (!settings) throw new Error("Settings (Goals) is undefined");

        let total = { cal: 0, pro: 0, fat: 0, carbo: 0, sodium: 0 };
        meals.forEach((m, idx) => {
            if (!m.foods && m.type !== 'Water') console.warn("Meal missing foods:", idx);
            (m.foods || []).forEach(f => {
                if (!f) return;
                total.cal += (f.cal || 0);
                total.pro += parseFloat(f.pro || 0);
                total.fat += parseFloat(f.fat || 0);
                total.carbo += parseFloat(f.carbo || 0);
        ...
        // 2. Fixed Header
        const header = document.createElement('div');
        header.style.cssText = 'position:fixed; top:0; left:0; width:100%; z-index:2000; background:#ffffff; border-bottom:1px solid #eee; display:grid; grid-template-columns:1fr auto 1fr; align-items:center; padding:15px 20px; padding-top:calc(15px + env(safe-area-inset-top));';

        // Check t() availability
        if (typeof window.t !== 'function') throw new Error("window.t is not a function");

        header.innerHTML = `
            <div style="font-size:20px; font-weight:800; color:black;">${window.t ? window.t('header.diet_log') : 'Diet Log'}</div>
            <div style="display:flex; align-items:center; gap:8px;">
                <button onclick="changeDate(-1)" style="background:none; border:none; padding:5px; color:var(--ios-blue); cursor:pointer;"><i data-lucide="chevron-left" style="width:20px;"></i></button>
                <button onclick="openCalendarModal()" style="background:#f2f4f7; border:none; padding:8px 12px; border-radius:18px; color:black; font-weight:700; font-size:15px; display:flex; align-items:center; gap:6px; cursor:pointer;">
                    ${state.date} 
        ...
        // Re-def helpers within scope just in case (though global should work)
        const getHealthColor = (current, target, type) => {
            if (!target) return '#FF6B6B';
            const pct = (current / target) * 100;
            const ranges = {
                calories: { healthy: [80, 120], warning: [50, 150] },
                water: { healthy: [80, 120], warning: [50, 150] },
                ...
                carbs: { healthy: [70, 130], warning: [50, 160] },
                fat: { healthy: [70, 130], warning: [50, 160] },
                sodium: { healthy: [0, 100], warning: [0, 150] }
            };
        ...
        const createNutrientDonut = (icon, label, current, target, unit, type) => {
            const isSodiumUnknown = (type === 'sodium' && current < 10);
            const safeTarget = target || 1;
            const pct = isSodiumUnknown ? 0 : Math.round((current / safeTarget) * 100);
            const deg = Math.min(360, (pct / 100) * 360);
            const color = isSodiumUnknown ? '#999' : getHealthColor(current, safeTarget, type);
            const size = 70;

            const displayText = isSodiumUnknown
                ? `<div style="font-size:11px; color:#999; font-weight:600; margin-bottom:1px; white-space:nowrap;">${label}</div><div style="font-size:16px; font-weight:800; color:#999; line-height:1.2;">—</div><div style="font-size:11px; color:#aaa; font-weight:500;">/ ${target} ${unit}</div>`
                : `<div style="font-size:11px; color:gray; font-weight:600; margin-bottom:1px; white-space:nowrap;">${label}</div><div style="font-size:16px; font-weight:800; color:#333; line-height:1.2;">${Math.round(current)}</div><div style="font-size:11px; color:gray; font-weight:500;">/ ${target} ${unit}</div>`;

            const iconDisplay = isSodiumUnknown
                ? `<i data-lucide="help-circle" style="width:18px; color:#999; margin-bottom:2px;"></i><span style="font-size:11px; font-weight:700; color:#999;">?</span>`
                : `<i data-lucide="${icon}" style="width:18px; color:${color}; margin-bottom:2px;"></i><span style="font-size:11px; font-weight:700; color:${color};">${pct}%</span>`;
            ...
            return `<div style="display:flex; align-items:center; gap:8px;">
                    ...
                    <div style="display:flex; flex-direction:column; justify-content:center; flex:1; min-width:0;">${displayText}</div>
                </div>`;
        };

        dashboard.innerHTML = `
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; margin-bottom:20px;">
                ${createNutrientDonut('flame', window.t('settings.target_cal'), total.cal, settings.targetCal, 'kcal', 'calories')}
                ${createNutrientDonut('droplet', window.t('settings.target_water'), water, settings.targetWater, 'ml', 'water')}
                ${createNutrientDonut('salt', window.t('settings.target_sodium'), total.sodium, settings.targetSodium || 2000, 'mg', 'sodium')}
            </div>
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; padding-top:15px; border-top:1px solid #f5f5f5;">
                ${createNutrientDonut('beef', window.t('settings.target_pro'), total.pro, settings.targetPro, 'g', 'protein')}
                ${createNutrientDonut('wheat', window.t('settings.target_carb'), total.carbo, settings.targetCarb, 'g', 'carbs')}
                ${createNutrientDonut('droplets', window.t('settings.target_fat'), total.fat, settings.targetFat, 'g', 'fat')}
            </div>
        `;
        container.appendChild(dashboard);
        ...
        // 4. Timeline
        const listTitle = document.createElement('h3');
        listTitle.style.cssText = 'font-size:18px; margin-bottom:10px;';
        listTitle.innerHTML = 'Timeline';
        container.appendChild(listTitle);
        ...
        if (meals.length === 0) {
            ...
            meals.forEach((m, idx) => {
                ...
                if (m.type === 'Water') {
                    card.onclick = () => window.openWaterEditor && window.openWaterEditor(m.id);
                    card.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;"><div style="display:flex; align-items:center; gap:8px;"><div style="font-size:13px; font-weight:700; color:#555; background:#f0f0f0; padding:2px 6px; border-radius:4px;">${m.time}</div><span style="font-size:16px;">💧</span><div style="font-size:15px; font-weight:600;">Water</div></div><div style="font-size:14px; font-weight:600; color:#5AC8FA;">${m.amount} ml</div></div>`;
                } else {
                    const mCal = (m.foods || []).reduce((s, f) => s + (f.cal || 0), 0);
                    card.onclick = () => window.openMealEditor && window.openMealEditor(m.id);
                    const summary = (m.foods || []).map(f => f.name).join(', ');
                    const mealEmoji = m.type === 'Meal' ? '🍱' : (m.type === 'Snack' ? '🍪' : '☕');
                    card.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;"><div style="display:flex; align-items:center; gap:8px;"><div style="font-size:13px; font-weight:700; color:#555; background:#f0f0f0; padding:2px 6px; border-radius:4px;">${m.time}</div><span style="font-size:16px;">${mealEmoji}</span><div style="font-size:15px; font-weight:600;">${m.type}</div></div><div style="font-size:14px; font-weight:600;">${Math.round(mCal)} kcal</div></div><div style="font-size:13px; color:gray; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">${summary}</div>`;
                }
                mealList.appendChild(card);
            });
        }
        ...
        const addBtn = document.createElement('button');
        addBtn.className = 'btn';
        addBtn.style.cssText = 'margin-top:20px; margin-bottom:100px; background:rgba(0,122,255,0.1); color:var(--ios-blue);';
        addBtn.innerHTML = '+ Add Diet Log';
        addBtn.onclick = () => window.openAddDietMenu && window.openAddDietMenu();
        container.appendChild(addBtn);

    } catch (e) {
        console.error(e);
        alert('Render Error: ' + e.message + '\n' + e.stack);
        container.innerHTML += `<div style="color:red; padding:20px;">Error loading view: ${e.message}</div>`;
    }
};

// --- V25 Fix Final: Robust Goal Logic (Append) ---

// 1. Hardened getGoalsForDate (Never returns undefined)
window.getGoalsForDate = (dateStr) => {
    const defaults = {
        targetCal: 2500,
        targetPro: 160,
    ...
    try {
        ...
        console.error("Goals Retrieval Error:", e);
        return defaults;
    }
};

// 2. Safe saveAllSettings (Prevents corruption)
window.saveAllSettings = () => {
    if (!window.tempSettingsState || !window.tempSettingsState.settings) {
        alert("오류: 설정 상태를 불러올 수 없습니다. 앱을 새로고침해주세요.");
        return;
    }
