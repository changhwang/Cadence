...
function renderWorkoutBuilder(container) {
    ...
    header.innerHTML = `
        <h1 style="margin:0; font-size:28px;">Daily Log</h1>
        <button onclick="openDatePicker()" style="background:none; border:none; color:var(--ios-blue); padding:10px;">
    ...
    stats.innerHTML = `
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr 1fr; text-align:center; gap:5px;">
             ...
             <div><div style="font-size:18px; font-weight:800;">${(totalVol / 1000).toFixed(1)}k</div><div style="font-size:11px; opacity:0.8;">Vol (kg)</div></div>
             <div><div style="font-size:18px; font-weight:800;">${totalCardio}</div><div style="font-size:11px; opacity:0.8;">Cardio (min)</div></div>
        ...
        current.forEach((ex, idx) => {
            html += `
                <div style="display:flex; align-items:center; padding:12px; border-bottom:1px solid #eee;">
                    ...
                    <label for="del_check_${idx}" style="font-size:16px; flex:1;">${ex}</label>
                </div>
            `;
        });
        html += `
...
window.openWorkoutDetail = (planIdx) => {
    ...
    if (history) {
        historyHtml = `
            <div style="background:#f0f8ff; padding:12px; border-radius:12px; margin-bottom:20px; border:1px solid #cce5ff;">
                <div style="font-size:12px; font-weight:700; color:#004085; margin-bottom:5px;">LAST SESSION (${history.date})</div>
                <div style="font-size:14px; color:#0056b3;">${history.summary}</div>
            </div>
        `;
    }
        ...
        exLogs.forEach((l, idx) => {
            logHtml += `
                <div style="display:flex; justify-content:space-between; align-items:center; background:#f9f9f9; padding:10px; border-radius:8px;">
                    ...
                    <div style="font-weight:700;">${l.wt}kg x ${l.reps}</div>
        ...
        <div style="margin-bottom:20px;">
            ...
            <div id="logArea" style="display:none; margin-top:20px; background:#fff3cd; padding:15px; border-radius:12px;">
                ...
                <div style="display:flex; gap:10px; margin-bottom:15px;">
                    <div style="flex:1;">
                        <label style="font-size:12px; color:gray; display:block; margin-bottom:5px;">Weight (kg)</label>
                        <input type="number" id="logWt" placeholder="0" style="width:100%; padding:15px; border:2px solid var(--ios-blue); border-radius:12px; font-size:24px; text-align:center; font-weight:700;">
                    </div>
                    <div style="flex:1;">
    ...
    // Restore Timer State
    if (state.timer.mode === 'work' && state.timer.exercise === exName) {
        updateTimerUI('work');
    } else if (state.timer.mode === 'rest') {
        updateTimerUI('rest');
    }
};
...
function getExerciseHistory(exName) {
    ...
    for (let i = 1; i < 30; i++) {
        ...
        const logs = Store.get(`workout_${dateStr}`, {});
        if (logs[exName] && logs[exName].length > 0) {
            // Found it
            const lastLog = logs[exName][logs[exName].length - 1]; // Last set
            const bestSet = logs[exName].reduce((best, curr) => (parseInt(curr.wt) > parseInt(best.wt) ? curr : best), logs[exName][0]);

            return {
                date: dateStr,
                summary: `${bestSet.wt}kg x ${bestSet.reps} (Best) / ${logs[exName].length} sets`
            };
        }
    }
    return null;
