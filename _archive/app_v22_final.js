...
window.removePlanItem = (idx) => {
    ...
    if (idx >= 0 && idx < plan.length) {
        ...
        setTimeout(() => {
            window.openBulkManagement();
            render();
        }, 50); // Slight delay to ensure modal transition clean
    }
};
...
function renderDietView(container) {
    ...
    // 1. Stats Calculation (V21: Added Sodium)
    ...
    meals.forEach(m => {
        m.foods.forEach(f => {
            ...
            total.sodium += parseFloat(f.sodium || 0); // V21: Track sodium
        });
    });
    ...
    const createNutrientDonut = (icon, label, current, target, unit, type) => {
        ...
        const displayText = isSodiumUnknown
            ? `<div style="font-size:11px; color:#999; font-weight:600; margin-bottom:2px;">${label}</div>
               <div style="font-size:12px; font-weight:600; color:#999;">— / ${target} ${unit}</div>
               <div style="font-size:10px; color:#aaa;">(미기록)</div>`
            : `<div style="font-size:12px; color:gray; font-weight:600; margin-bottom:2px;">${label}</div>
               <div style="font-size:14px; font-weight:700; color:#333;">${Math.round(current)} / ${target} ${unit}</div>`;
    ...
    dashboard.innerHTML = `
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; margin-bottom:20px;">
            ${createNutrientDonut('flame', 'Calories', total.cal, settings.targetCal, 'kcal', 'calories')}
            ${createNutrientDonut('droplet', 'Water', water, settings.targetWater, 'ml', 'water')}
            ${createNutrientDonut('salt', 'Sodium', total.sodium, settings.targetSodium || 2000, 'mg', 'sodium')}
        </div>

        <!-- V21: Row 2 - Macros -->
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; padding-top:15px; border-top:1px solid #f5f5f5;">
            ${createNutrientDonut('beef', 'Protein', total.pro, settings.targetPro, 'g', 'protein')}
            ${createNutrientDonut('wheat', 'Carbs', total.carbo, 300, 'g', 'carbs')}
            ${createNutrientDonut('droplets', 'Fat', total.fat, 80, 'g', 'fat')}
        </div>
    `;
