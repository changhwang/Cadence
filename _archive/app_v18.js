    ...
    dashboard.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            ...
            <div style="display:flex; align-items:center; gap:15px;">
                 <div style="position:relative; width:60px; height:60px; border-radius:50%; background:conic-gradient(var(--ios-blue) ${calDeg}deg, #E5E5EA 0deg); display:flex; align-items:center; justify-content:center;">
                    <div style="width:50px; height:50px; background:white; border-radius:50%; display:flex; align-items:center; justify-content:center;">
                        <i data-lucide="flame" style="width:20px; color:var(--ios-blue);"></i>
                    </div>
                 </div>
                 <div>
                    <div style="font-size:18px; font-weight:800;">${Math.round(total.cal)}</div>
                    <div style="font-size:12px; color:gray;">Remaining: ${remaining}</div>
                 </div>
            </div>
            ...
            <div style="text-align:right;">
                <div style="display:flex; align-items:center; justify-content:flex-end; gap:5px; margin-bottom:4px;">
                     <i data-lucide="droplet" style="width:16px; color:#5AC8FA;"></i>
                     <span style="font-weight:700; font-size:16px; color:#5AC8FA;">${water}ml</span>
                </div>
                <div style="display:flex; gap:5px;">
                     <button onclick="logWater(-250)" style="width:28px; height:28px; border-radius:14px; border:1px solid #eee; background:white;">-</button>
                     <button onclick="logWater(250)" style="width:28px; height:28px; border-radius:14px; border:1px solid #eee; background:white;">+</button>
                </div>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:10px;">
            ${createMiniMacro("Protein", total.pro, settings.targetPro, "var(--ios-blue)")}
            ${createMiniMacro("Carbs", total.carbo, 300, "var(--ios-orange)")}
            ${createMiniMacro("Fat", total.fat, 80, "#FFCC00")}
        </div>
    `;
    container.appendChild(dashboard);

    // Helper for Mini Macro
    function createMiniMacro(label, current, target, color) {
        const pct = Math.min(100, (current / target) * 100);
        return `
            <div>
                <div style="font-size:12px; color:gray; margin-bottom:2px;">${label}</div>
                <div style="font-size:13px; font-weight:600; margin-bottom:4px;">${Math.round(current)}/${target}g</div>
    ...
    modal.innerHTML = `
        <div style="background:white; padding:20px; border-radius:20px; width:80%; max-width:300px; text-align:center;">
            ...
            <div style="display:grid; gap:10px;">
                ...
                <button onclick="closeModal(); alert('Please use the dashboard buttons for water');" style="padding:15px; border:1px solid #eee; background:white; border-radius:12px; font-size:16px; font-weight:600;">💧 Water</button>
            </div>
