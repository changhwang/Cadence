...
window.openFoodSelector = () => {
    ...
    const renderList = (filter) => {
        ...
        <div style="display:flex; gap:5px; overflow-x:auto; padding-bottom:10px; margin-bottom:10px;">
            ${cats.map(c => `<button onclick="updateFoodList('${c}')" style="white-space:nowrap; padding:6px 12px; border-radius:15px; border:none; background:${filter === c ? 'var(--ios-blue)' : '#eee'}; color:${filter === c ? 'white' : 'black'};">${t('muscles.' + c.toLowerCase())}</button>`).join('')}
            </div>
        <input type="text" id="foodSearchInput" placeholder="${t('workout.search_food')}" style="width:100%; padding:12px; border-radius:10px; border:1px solid #ddd; margin-bottom:10px;">
...
window.openWorkoutDetail = (planIdx) => {
    ...
    modal.innerHTML = `
        <div style="margin-bottom:15px;">
            ...
            <!-- LOG FORM (Initially Hidden or shown based on state) -->
            <div id="logFormArea" style="display:block; margin-top:10px; border-top:1px solid #eee; padding-top:15px;">
                <div style="font-size:13px; font-weight:700; margin-bottom:10px; color:#333;">Log Next Set</div>
                <div style="display:flex; gap:10px; margin-bottom:10px;">
                    <div style="flex:1;">
...
window.finishWorkSet = () => {
    ...
    // UI Update: Ready to Save & Rest
    const btn = document.getElementById('mainTimerBtn');
    const status = document.getElementById('timerStatus');
    const display = document.getElementById('timerDisplay');

    if (btn) {
        btn.innerText = t('workout.save_rest');
        btn.style.background = 'var(--ios-blue)';
        btn.onclick = () => saveSetLog(state.timer.exercise);
    }
    if (status) {
        status.innerText = "SET COMPLETED";
    ...
    // Highlight inputs
    document.getElementById('logWt').focus();
};
...
window.saveSetLog = (exName) => {
    ...
    const durInput = document.getElementById('logDuration').value;

    if (!wtInput || !repsInput) return alert("Please enter weight and reps");
    ...
    logs[exName].push({
        wt,
        reps: repsInput,
        duration: durInput, // Save duration
        timestamp: Date.now()
    });
    Store.set(key, logs);
...
window.startRestTimer = () => {
    ...
    state.timer.intervalId = setInterval(() => {
        ...
        if (state.timer.seconds <= 0) {
            ...
            document.getElementById('timerStatus').innerText = "REST FINISHED";
        }
    }, 1000);
};
...
window.skipRest = () => {
    ...
    if (btn) {
        btn.innerText = t('workout.start_set');
        btn.style.background = 'var(--ios-blue)';
        btn.onclick = () => startSet(state.timer.exercise, state.timer.targetRest);
    }

    // Clear inputs slightly for next set convenience (optional, maybe keep w/r?)
    // document.getElementById('logReps').value = ''; // Keep weight, clear reps? No, keep both is better.
};
...
window.updateTimerUI = (mode, exName) => {
    ...
    const status = document.getElementById('timerStatus');
    const display = document.getElementById('timerDisplay');
    if (!btn || !status || !display) return;

    if (mode === 'work') {
        // Work Mode UI
        btn.innerText = t('workout.finish_set');
        btn.style.background = 'var(--ios-green)';
        btn.onclick = finishWorkSet;
        ...
        // Rest Mode UI
        btn.innerText = t('workout.skip_rest');
        btn.style.background = '#888'; // Gray for skip
