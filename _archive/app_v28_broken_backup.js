...
window.renderMyBodyView = (container) => {
    ...
    statsCard.innerHTML = `
       <h3 style="margin:0 0 15px 0; font-size:20px; font-weight:800; color:#222;">${t('body.activity_volume')}</h3>
       ${drawVolumeChart(30)}
    `;
...
window.getExerciseIndex = () => {
    ...
    // 2. Scan Recent Logs for Custom Exercises (Last 90 days)
    // This ensures exercises not in DB (e.g. custom names) appear in the list.
    const today = new Date();
    const startD = new Date(today);
    startD.setDate(today.getDate() - 90);
    const start = startD.toISOString().split('T')[0];
    ...
    for (let d = new Date(start); d <= new Date(end); d.setDate(d.getDate() + 1)) {
        ...
        if (logs) {
            Object.keys(logs).forEach(name => {
                if (name !== 'cardio' && Array.isArray(logs[name])) {
    ...
    // Helper to get localized name if possible
    const getDisp = (k) => {
        if (window.getDisplayName) return window.getDisplayName(k);
        return k;
    };
    ...
    return sortedKeys.map(k => {
        return {
            id: k,
            name: getDisp(k), // localized name
            group: getSharedMuscleGroup(k) // robust group lookup
        };
    });
};
...
const getSharedMuscleGroup = (name) => {
    ...
    if (window.EXERCISE_DB) {
        ...
        for (const [key, val] of entries) {
            ...
            // e.g. Log: "Bench Press" -> Key: "벤치 프레스 (Barbell Bench Press)"
...
const getMg = (info) => {
    if (!info || !info.muscleGroup) return 'Other';
    return info.muscleGroup.charAt(0).toUpperCase() + info.muscleGroup.slice(1);
};
    ...
    container.innerHTML = `
        ...
        <div style="padding:15px; background:#f9f9f9;">
            <input type="text" placeholder="Search exercises..." 
                ...
                style="width:100%; padding:12px; border:1px solid #ddd; border-radius:12px; font-size:16px; outline:none;">
        </div>
