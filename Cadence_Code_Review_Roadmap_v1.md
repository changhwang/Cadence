...
### 8-5. 남은 정리 대상 (다음 라운드)
- 식단 수정 모달에서 수량을 바꿔도 kcal/단백/탄수/지방 입력칸은 자동 갱신되지 않음(미량영양소만 재계산). 추가 모달(`openDietAddDetailModal`)에는 자동 동기화가 있으니 동일하게 맞출 것.
- UI 상태가 store / 모듈 전역(BodyView·StatsView) / DOM 세 곳에 분산 — 규칙 정리 필요.
- 인라인 스타일/하드코딩 색상(DietView 건강도 색, 히트맵) → CSS 변수로 이관(다크모드 대비).
